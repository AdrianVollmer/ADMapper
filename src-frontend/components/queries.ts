    ...
    queries: [
      {
        ...
        query: `MATCH (u:User)-[e:Edge]->(g:Group) WHERE e.edge_type = 'MemberOf' AND g.object_id ENDS WITH '-512' RETURN u, e, g`,
      },
      {
        id: "all-dcs",
        name: "Domain Controllers",
        description: "All domain controller computers (SID -516)",
        query: `MATCH (c:Computer)-[e:Edge]->(g:Group) WHERE e.edge_type = 'MemberOf' AND g.object_id ENDS WITH '-516' RETURN c, e, g`,
      },
      {
        id: "domain-trusts",
        name: "Domain Trusts",
        description: "All domain trust relationships",
        query: `MATCH (a:Domain)-[e:Edge]->(b:Domain) WHERE e.edge_type = 'TrustedBy' RETURN a, e, b`,
      },
    ],
  },
  {
        ...
        queries: [
          {
            ...
            query: `MATCH (p)-[e:Edge]->(d:Domain) WHERE e.edge_type = 'DCSync' RETURN p, e, d`,
          },
          {
            id: "dcsync-non-da",
      ...
      },
      {
        ...
        queries: [
          ...
          },
          {
            ...
            query: `MATCH (p)-[e:Edge]->(t) WHERE e.edge_type = 'AllowedToDelegate' RETURN p, e, t`,
          },
          {
            id: "rbcd",
            name: "Resource-Based Constrained Delegation",
            description: "Principals that can RBCD to a target",
            query: `MATCH (p)-[e:Edge]->(t) WHERE e.edge_type = 'AllowedToAct' RETURN p, e, t`,
          },
        ],
      },
      {
        ...
        queries: [
          {
            ...
            query: `MATCH (p)-[e:Edge]->(t) WHERE e.edge_type = 'GenericAll' RETURN p, e, t`,
          },
          {
            id: "genericwrite",
            name: "GenericWrite Rights",
            description: "Principals with GenericWrite on objects",
            query: `MATCH (p)-[e:Edge]->(t) WHERE e.edge_type = 'GenericWrite' RETURN p, e, t`,
          },
          {
            id: "writedacl",
            name: "WriteDacl Rights",
            description: "Principals that can modify DACLs",
            query: `MATCH (p)-[e:Edge]->(t) WHERE e.edge_type = 'WriteDacl' RETURN p, e, t`,
          },
          {
            id: "writeowner",
            name: "WriteOwner Rights",
            description: "Principals that can change ownership",
            query: `MATCH (p)-[e:Edge]->(t) WHERE e.edge_type = 'WriteOwner' RETURN p, e, t`,
          },
        ],
      },
    ],
    ...
    queries: [
      {
        ...
        query: `MATCH (u:User)-[e:Edge]->(c:Computer) WHERE e.edge_type = 'HasSession' RETURN u, e, c`,
      },
      {
        id: "rdp-users",
        name: "RDP Access",
        description: "Users with RDP access to computers",
        query: `MATCH (u:User)-[e:Edge]->(c:Computer) WHERE e.edge_type = 'CanRDP' RETURN u, e, c`,
      },
      {
        id: "local-admins",
        name: "Local Admins",
        description: "Users with local admin rights",
        query: `MATCH (u:User)-[e:Edge]->(c:Computer) WHERE e.edge_type = 'AdminTo' RETURN u, e, c`,
      },
    ],
  },
  {
  ...
  },
  {
    ...
    queries: [
      ...
      },
      {
        ...
        query: `MATCH (t:CertTemplate)-[e:Edge]->(ca:EnterpriseCA) WHERE e.edge_type = 'PublishedTo' AND t.properties CONTAINS 'enrolleesuppliessubject' RETURN t, e, ca`,
      },
    ],
  },
  {
    ...
    queries: [
      ...
      },
      {
        ...
        name: "Shortest Paths to DA",
        description: "Shortest paths from users to Domain Admins (SID -512)",
        query: `MATCH p = (u:User)-[:Edge* SHORTEST 1..10]->(da:Group) WHERE da.object_id ENDS WITH '-512' RETURN p`,
      },
    ],
  },
];
