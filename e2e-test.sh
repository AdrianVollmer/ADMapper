...
show_usage() {
    ...
    echo ""
    echo "Options:"
    echo "  --backend <name>   Test specific backend: kuzu, crustdb, or all (default: all)"
...
parse_args() {
    ...
    case "$BACKEND" in
        ...
        *)
            ...
            echo "Valid backends: kuzu, crustdb, all"
            exit 1
            ;;
    esac
}
...
run_tests() {
    ...
    $COMPOSE_CMD -f e2e/docker-compose.yml build tests

    # Run the tests
    # Mount the binary and test data into the container
    local exit_code=0

    $COMPOSE_CMD -f e2e/docker-compose.yml run \
        --rm \
        -v "$ADMAPPER_BIN:/workspace/admapper:ro" \
        -v "$TEST_DATA:/workspace/test_data.zip:ro" \
        -e "ADMAPPER_BIN=/workspace/admapper" \
        tests \
        ./e2e/run-tests.sh /workspace/test_data.zip "$BACKEND" || exit_code=$?
