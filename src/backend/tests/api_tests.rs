...
/// assign_member_tiers (group-object query) must set tier 0 on the DA group
/// placeholder node itself, not only on its members. This covers the case where
/// importing users.json before groups.json leaves the DA group without a tier.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
