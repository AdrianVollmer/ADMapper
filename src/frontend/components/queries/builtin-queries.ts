  ...
  },
  {
    ...
    queries: [
      ...
      },
      {
        ...
        query: `MATCH (u:User) WHERE u.hasspn = true AND u.enabled = true MATCH p = shortestPath((u)-[*1..50]->(c:Computer)) RETURN p LIMIT 1000`,
      },
      {
        ...
        query: `MATCH (u:User) WHERE u.dontreqpreauth = true AND u.enabled = true MATCH p = shortestPath((u)-[*1..50]->(c:Computer)) RETURN p LIMIT 1000`,
      },
    ],
  },
  {
        ...
        queries: [
          {
            ...
            query: `MATCH p = (a)-[r]->(b) WHERE a.tier = 1 AND b.tier = 0 RETURN p LIMIT 1000`,
          },
          {
            id: "tier2-to-tier0",
            name: "Tier 2 → Tier 0",
            description: "Single-hop relationships from tier 2 nodes to tier 0 nodes",
            query: `MATCH p = (a)-[r]->(b) WHERE a.tier = 2 AND b.tier = 0 RETURN p LIMIT 1000`,
          },
          {
            id: "tier3-to-tier0",
            name: "Tier 3 → Tier 0",
            description: "Single-hop relationships from tier 3 nodes to tier 0 nodes",
            query: `MATCH p = (a)-[r]->(b) WHERE a.tier = 3 AND b.tier = 0 RETURN p LIMIT 1000`,
          },
          {
            id: "any-to-tier0",
            name: "Any → Tier 0",
            description: "All single-hop relationships reaching tier 0 from higher-numbered tiers",
            query: `MATCH p = (a)-[r]->(b) WHERE b.tier = 0 AND a.tier > 0 RETURN p LIMIT 1000`,
          },
        ],
      },
      {
        ...
        queries: [
          {
            ...
            query: `MATCH p = (a)-[r]->(b) WHERE a.tier = 2 AND b.tier = 1 RETURN p LIMIT 1000`,
          },
          {
            id: "tier3-to-tier1",
            name: "Tier 3 → Tier 1",
            description: "Single-hop relationships from tier 3 nodes to tier 1 nodes",
            query: `MATCH p = (a)-[r]->(b) WHERE a.tier = 3 AND b.tier = 1 RETURN p LIMIT 1000`,
          },
          {
            id: "any-to-tier1",
            name: "Any → Tier 1",
            description: "All single-hop relationships reaching tier 1 from higher-numbered tiers",
            query: `MATCH p = (a)-[r]->(b) WHERE b.tier = 1 AND a.tier > 1 RETURN p LIMIT 1000`,
          },
        ],
      },
      {
        ...
        queries: [
          {
            ...
            query: `MATCH p = (a)-[r]->(b) WHERE a.tier = 3 AND b.tier = 2 RETURN p LIMIT 1000`,
          },
          {
            id: "any-to-tier2",
            name: "Any → Tier 2",
            description: "All single-hop relationships reaching tier 2 from higher-numbered tiers",
            query: `MATCH p = (a)-[r]->(b) WHERE b.tier = 2 AND a.tier > 2 RETURN p LIMIT 1000`,
          },
        ],
      },
      {
        ...
        queries: [
          {
            ...
            query: `MATCH p = (a)-[r]->(b) WHERE a.tier IS NOT NULL AND b.tier IS NOT NULL AND a.tier > b.tier RETURN p LIMIT 1000`,
          },
        ],
      },
    ],
    ...
    queries: [
      {
        ...
        query: `MATCH (u:User), (da:Group), p = shortestPath((u)-[*1..50]->(da)) WHERE da.objectid ENDS WITH '-512' AND ALL(r IN relationships(p) WHERE r.exploit_likelihood >= 0.5) RETURN p LIMIT 1000`,
      },
    ],
  },
];
